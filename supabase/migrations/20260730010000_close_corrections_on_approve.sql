-- Approving a case should close out every open correction across all of
-- its revisions, so trainer/trainee stop seeing "open" items on a case
...
begin
  ...
  if approve_package then
    ...
    -- Close every open correction across all revisions of this case —
    -- approval means the package is done, so nothing should still read
    -- "open" for either the trainer or the trainee.
